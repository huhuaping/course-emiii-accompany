...
format:
  ...
  docx:
    toc: true
# link to the bibliography file, which is in the parent directory's bib folder
# recommended to use Zotero to manage your bibliography and export it as a .bib file in the bib folder
bibliography: ../bib/endogeneity.bib
# choose bibliography style from Zotero.org's CSL style repository: https://github.com/citation-style-language/styles
# Or use your own custom style by providing the path to the .csl file
# Here we use the econometrica style as an example in directory: ../bib/
csl: ../bib/econometrica.csl
---

**Abstract**:This paper is a simple introduction to the two-stage least squares (**TSLS**) method, which is used to estimate the parameters of a model when there are endogenous variables. The paper provides an overview of the TSLS method, including its assumptions and limitations, and discusses its applications in various fields such as economics, finance, and social sciences. The paper also presents a case study using real-world data to illustrate the application of the TSLS method and its advantages over traditional ordinary least squares (OLS) regression.

**Keywords**:: something powerful


## Introduction

This topic is important. balalaba

## literature

@abadie2002 babalalba @andrews2019 say iv is powerful.
...
```{r}
# filter mroz data set: women with positive wage and in labor force
moroz_spl <- mroz %>%
  filter(wage>0, inlf==1) %>%
  select(wage, educ, exper, expersq, fatheduc, motheduc, huseduc)
```
...
$$
\begin{aligned}
Y_i &= \beta_1 + \beta_2X_i +\beta_3 X_i^2 \\
\hat{\beta}_{OLS} &= (XX')^{-1}XY'
\end{aligned}
$$
